...
function insert()
{
    ...
    $sql = "INSERT INTO board (board_pw, board_title,
            board_content, board_user, board_date)
            values ('" . $board_pw . "','" . $board_title . "',
                    '" . $board_content . "','" . $board_user . "',now())";
...
function selectOne($key){
    $conn = dbConnect();
    $sql = "SELECT board_no, board_title, board_user,
                     board_date, board_content FROM board where board_no='".$key."'";
